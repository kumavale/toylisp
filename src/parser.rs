...
pub fn eval(tokens: &mut Parser, env: &mut Env) -> Result<i32, String> {
    ...
    match tokens.peek() {
        ...
        Some(Token::Ne) | Some(Token::Eq) | Some(Token::Lt) | Some(Token::Le) | Some(Token::Gt)
        | Some(Token::Ge) => {
            let op = tokens.next().unwrap();
...
mod tests {
    ...
    fn test_eval() {
        ...
        assert_eq!(
            eval(&mut tokenize("(/ 8 4 0)"), &mut env),
            Err("failed calculation".to_string())
        );

        // 入れ子
        assert_eq!(eval(&mut tokenize("(+ (* 5 3) 5)"), &mut env), Ok(20));
        assert_eq!(
            eval(&mut tokenize("(+ (* 5 (- 4 2) 3) 5)"), &mut env),
            Ok(35)
        );
        assert_eq!(
            eval(&mut tokenize("(+ (* 5 3 (- 4 2)) 5)"), &mut env),
            Ok(35)
        );
        ...
        assert_eq!(
            eval(&mut tokenize("(setq x 3) (setq y 6) (+ x y)"), &mut env),
            Ok(9)
        );
        assert_eq!(
            eval(&mut tokenize("(setq x 3 y 6) (+ x y)"), &mut env),
            Ok(9)
        );
        assert_eq!(
            eval(&mut tokenize("(setq x 42 x 21) (x)"), &mut env),
            Ok(21)
        );
        assert_eq!(
            eval(
                &mut tokenize("(setq x 10) (setq y (+ x 100)) (y)"),
                &mut env
            ),
            Ok(110)
        );
        ...
        assert_eq!(
            eval(
                &mut tokenize("(defun add (a b) (+ a b)) (add 1 2)"),
                &mut env
            ),
            Ok(3)
        );
        assert_eq!(
            eval(
                &mut tokenize("(defun add (a b) (+ a b)) (add (add 1 2) 3)"),
                &mut env
            ),
            Ok(6)
        );
        assert_eq!(
            eval(
                &mut tokenize("(defun add (a b) (+ a b)) (add 1 (add 2 3))"),
                &mut env
            ),
            Ok(6)
        );
        assert_eq!(
            eval(
                &mut tokenize("(defun foo (a b c) (+ a (* c 2) b)) (foo 1 2 3)"),
                &mut env
            ),
            Ok(9)
        );
        ...
        assert_eq!(
            eval(&mut tokenize("(if (+ 1 2) (+ 1 10) (+ 1 100))"), &mut env),
            Ok(11)
        );
        assert_eq!(
            eval(
                &mut tokenize(
                    "(defun fib (x) (if (<= x 1) 1 (+ (fib (- x 1)) (fib (- x 2))))) (fib 9)"
                ),
                &mut env
            ),
            Ok(55)
